...
const razasFiltradas = computed(() => {
  ...
  return todasLasRazas.value.filter(
    (r) => r.id_especie === form.value.id_especie,
  );
});
...
function validar() {
  if (!form.value.nombre.trim())
    return t("pets.name") + " " + t("common.required");
  ...
  if (esMestizo.value) {
    ...
    if (form.value.id_raza === form.value.id_raza2)
      return "Las dos razas no pueden ser iguales";
  }
  return null;
}
        ...
        <div class="card-body nm-inner">
          ...
          <div class="nm-form">
            ...
            <div class="input-group">
              ...
              <div v-else class="pill-group">
                <button
                  ...
                  :class="[
                    'pill-btn',
                    { 'pill-btn--on': form.id_especie === esp.id },
                  ]"
                  @click="
                    form.id_especie = esp.id;
                    onEspecieChange();
                  "
                >
                  {{ especieLabel(esp.especie) }}
                </button>
              </div>
            </div>
            ...
            >
              ...
              <div
                class="mini-toggle"
                :class="{ 'mini-toggle--on': esMestizo }"
              >
              ...
              <div class="input-group">
                ...
                <div class="pill-group">
                  <button
                    type="button"
                    :class="[
                      'pill-btn pill-btn--macho',
                      { 'pill-btn--on': form.genero === 'macho' },
                    ]"
                  ...
                  <button
                    type="button"
                    :class="[
                      'pill-btn pill-btn--hembra',
                      { 'pill-btn--on': form.genero === 'hembra' },
                    ]"
            ...
            <div class="nm-actions">
              ...
              >
                <span
                  v-if="guardando"
                  class="spinner"
                  style="width: 15px; height: 15px; border-width: 2px"
                />
                <span v-else>{{
                  modoEdicion ? t("pets.saveChanges") : t("pets.register")
                }}</span>
              </button>
            </div>
          </div>
...
.nm-foto-area:hover .nm-foto-circle {
...
.nm-foto-area:hover .nm-foto-edit-overlay {
  opacity: 1;
}
.hidden-input {
  display: none;
