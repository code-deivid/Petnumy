...
async function cargarCitas() {
  ...
  loading.value = false;
  if (!ok) {
    error.value = data.message || "Error al cargar las citas";
    return;
  }
...
const BADGE_COLOR = computed(() => ({
  pendiente: {
    bg: "#FFF3E0",
    color: "#E65100",
    label: t("appointments.status.pendiente"),
  },
  confirmada: {
    bg: "#E0F1EE",
    color: "#4AADA5",
    label: t("appointments.status.confirmada"),
  },
  completada: {
    bg: "#E8F0FC",
    color: "#3A5FA0",
    label: t("appointments.status.completada"),
  },
  cancelada: {
    bg: "#FDEAEA",
    color: "#D95F5F",
    label: t("appointments.status.cancelada"),
  },
}));
    ...
    <template v-else>
      ...
      <div v-if="citas.length === 0" class="mc-empty card">
        ...
        <button
          type="button"
          class="btn btn-teal"
          @click="router.push({ name: 'clinicas' })"
        >
          <Icon
            style="flex-shrink: 0"
            :icon="$icons.location"
              ...
              <div class="mc-card-inner">
                ...
                <div class="mc-info">
                  <div class="mc-info-row1">
                    <span class="mc-mascota">{{
                      cita.mascota?.nombre || "—"
                    }}</span>
                    <span
                      class="mc-badge"
                      :style="{
                        background: BADGE_COLOR[cita.estado]?.bg,
                        color: BADGE_COLOR[cita.estado]?.color,
                      }"
                ...
                <div class="mc-accion">
                  <template v-if="confirmCancelar === cita.id">
                    <div class="mc-confirm">
                      <p class="mc-confirm-txt">
                        {{ t("appointments.cancelQuestion") }}
                      </p>
                      <div style="display: flex; gap: 0.4rem">
                        <button
                          type="button"
                          class="btn btn-ghost btn-sm"
                          @click="confirmCancelar = null"
                        >
                          {{ t("common.no") }}
                        </button>
                        <button
                          type="button"
                          class="btn btn-sm mc-btn-cancelar-confirm"
                          @click="ejecutarCancelar(cita)"
                        >
                          <span
                            v-if="cancelando === cita.id"
                            class="spinner"
