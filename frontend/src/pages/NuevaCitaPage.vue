...
async function elegirClinica(clinica) {
  ...
  servicios.value = resClinica.ok
    ? resClinica.data.clinica.servicios || []
    : [];
...
async function confirmarCita() {
  ...
  if (!mascotaSel.value) {
    formError.value = "Selecciona una mascota";
    return;
  }
  if (!fechaHora.value) {
    ...
    <div class="ncita-head">
      ...
      <div class="steps flex items-center gap-2">
        <div
          ...
          :class="[
            'step',
            { 'step--active': paso === n, 'step--done': paso > n },
          ]"
        >
          <span class="step-num">{{ paso > n ? "✓" : n }}</span>
          <span class="step-label">{{
            ["Clínica", "Servicio", "Confirmar"][n - 1]
          }}</span>
        </div>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <Transition name="fade" mode="out-in">
        <div v-if="paso === 2" key="paso2">
          <p class="paso-desc">
            Escoge veterinario y tipo de consulta en
            <strong>{{ clinicaSel?.nombre }}</strong>
          </p>
            ...
            <div>
              ...
              <div class="opciones-list">
                <button
                  ...
                  :class="[
                    'opcion-item card',
                    { 'opcion-item--selected': vetSel?.id === vet.id },
                  ]"
                  @click="vetSel = vet"
                >
                  <div class="card-body flex items-center gap-3">
                    <div class="vet-avatar-sm">
                      {{ (vet.nombre[0] || "") + (vet.apellidos?.[0] || "") }}
                    </div>
                    <div class="flex flex-col gap-0">
                      <span class="opcion-nombre"
                        >{{ vet.nombre }} {{ vet.apellidos }}</span
                      >
                      <span v-if="vet.especialidad" class="opcion-sub">{{
                        vet.especialidad
                      }}</span>
                    </div>
                ...
                <p v-if="veterinarios.length === 0" class="empty-sub">
            ...
            <div>
              ...
              <div class="opciones-list">
                <button
                  ...
                  :class="[
                    'opcion-item card',
                    { 'opcion-item--selected': servicioSel?.id === srv.id },
                  ]"
                ...
                >
                  <div class="card-body flex items-center gap-3">
                    <div class="flex flex-col gap-0" style="flex: 1">
                      ...
                      <span class="opcion-sub"
                        >{{ srv.duracion_minutos }} min
                        <template v-if="formatPrecio(srv.precio)">
                          · {{ formatPrecio(srv.precio) }}</template
                        >
                      </span>
                    </div>
                ...
                <p v-if="servicios.length === 0" class="empty-sub">
          ...
          <div class="card resumen-card">
            <div class="card-body flex flex-col gap-1">
              ...
              <p class="resumen-item">
                <strong>Clínica:</strong> {{ clinicaSel?.nombre }}
              </p>
              <p class="resumen-item">
                <strong>Veterinario:</strong> Dr. {{ vetSel?.nombre }}
                {{ vetSel?.apellidos }}
              </p>
              <p class="resumen-item">
                <strong>Servicio:</strong> {{ servicioSel?.nombre }} ({{
                  servicioSel?.duracion_minutos
                }}
                min)
              </p>
            </div>
          ...
          <div class="paso3-form">
            ...
            <div class="input-group">
              ...
              <div class="mascotas-selector">
                <button
                  ...
                  :class="[
                    'mascota-chip',
                    { 'mascota-chip--selected': mascotaSel?.id === m.id },
                  ]"
                  @click="mascotaSel = m"
                >
                  {{ m.nombre }}
                </button>
                <p v-if="mascotas.length === 0" class="empty-sub">
                  No tienes mascotas.
                  <RouterLink :to="{ name: 'mis-mascotas' }" class="link-inline"
                    >Añade una primero</RouterLink
                  >
                </p>
              </div>
            </div>
            ...
            <div class="input-group">
              <label class="label" for="notas"
                ...
                <span class="label-opt">(opcional)</span></label
              >
              <textarea
                id="notas"
                v-model="notas"
...
.opcion-item--selected {
  ...
  box-shadow: 0 0 0 3px rgba(92, 200, 192, 0.12);
}
.opcion-item .card-body {
