...
function traducirEspecie(valor = "") {
  ...
  if (v.includes("perro") || v.includes("dog") || v.includes("gos"))
    return t("common.dog");
  if (v.includes("gato") || v.includes("cat") || v.includes("gat"))
    return t("common.cat");
  return valor || "";
}
...
async function cargarDatos() {
  ...
  loading.value = false;
  if (!ok) {
    error.value = data.message || "No se encontró la mascota";
    return;
  }
...
function diasRestantes(proxima) {
  ...
  const diff = Math.ceil(
    (new Date(proxima) - Date.now()) / (1000 * 60 * 60 * 24),
  );
...
const iniciales = computed(() =>
  (mascota.value?.nombre?.[0] || "").toUpperCase(),
);

const edad = computed(() => {
  if (!mascota.value?.nacimiento) return null;
  const m = Math.floor(
    (Date.now() - new Date(mascota.value.nacimiento)) /
      (1000 * 60 * 60 * 24 * 30.44),
  );
...
const fechaNac = computed(() => {
  if (!mascota.value?.nacimiento) return null;
  return new Date(mascota.value.nacimiento).toLocaleDateString(
    dateLocale.value,
    {
      day: "numeric",
      month: "long",
...
const badgeClass = {
  puesta: "bv--puesta",
  pendiente: "bv--pendiente",
  retrasada: "bv--retrasada",
};
const badgeLabel = computed(() => ({
  puesta: t("vaccines.statusDone"),
  pendiente: t("vaccines.statusPending"),
  retrasada: t("vaccines.statusLate"),
}));
...
<template>
  <div class="md-page page-container">
    ...
    <button
      type="button"
      class="md-back"
      @click="router.push({ name: 'mis-mascotas' })"
    >
    ...
    <template v-else-if="mascota">
            ...
            <div class="md-foto-name">
              ...
              <div class="md-badges-row">
                <span
                  ...
                  :class="
                    mascota.raza.especie.especie.toLowerCase() === 'perro' ||
                    mascota.raza.especie.especie.toLowerCase() === 'dog' ||
                    mascota.raza.especie.especie.toLowerCase() === 'gos'
                      ? 'md-especie-pill--perro'
                      : 'md-especie-pill--gato'
                  "
                >
              ...
              <p class="md-raza-lbl">
                ...
                <template v-if="mascota.es_mestizo && mascota.raza_secundaria">
                  ...
                  >
                    + </span
                  >{{ mascota.raza_secundaria.nombre }}
                </template>
              </p>
            </div>
          </div>
          ...
          <div class="md-datos-col">
            ...
            <div class="md-datos-grid">
              <div v-if="fechaNac" class="md-dato">
                ...
                <div>
                  <span class="md-dato-key">{{
                    t("petDetail.birthdate")
                  }}</span>
              ...
              <div v-if="mascota.microchip" class="md-dato">
                ...
                <div>
                  ...
                  <span class="md-dato-val md-dato-val--mono">{{
                    mascota.microchip
                  }}</span>
                </div>
              </div>
      ...
      <section class="md-section">
        ...
        <div class="card vac-cartilla">
          <div class="vac-cartilla-head">
            ...
            <div class="vac-cartilla-info">
              ...
              <p class="vac-cartilla-sub">
                ...
                <strong>{{
                  vacunas.length > 0
                    ? fmt(vacunas[0].created_at)
                    : t("petDetail.noRegisters")
                }}</strong>
              </p>
            </div>
            <!-- Stats rápidas -->
            <div class="vac-stats">
              <div class="vac-stat">
                <span class="vac-stat-num">{{
                  vacunas.filter((v) => v.estado === "puesta").length
                }}</span>
                <span class="vac-stat-lbl">{{ t("petDetail.upToDate") }}</span>
              </div>
              <div class="vac-stat">
                <span class="vac-stat-num vac-stat-num--warn">{{
                  vacunas.filter((v) => v.estado === "pendiente").length
                }}</span>
                <span class="vac-stat-lbl">{{ t("petDetail.pending") }}</span>
              </div>
              <div class="vac-stat">
                <span class="vac-stat-num vac-stat-num--danger">{{
                  vacunas.filter((v) => v.estado === "retrasada").length
                }}</span>
          ...
          <div v-else-if="vacunas.length === 0" class="vac-empty">
            ...
            <p class="vac-empty-sub">
              {{ t("petDetail.addFirstVaccineFor") }}
          ...
          >
            ...
            <div class="vac-col-nombre" @click="abrirDetalle(vac)">
              <div class="vac-row-icon" :class="`vac-row-icon--${vac.estado}`">
                <Icon
                  class="vac-icon-emoji"
                  :icon="vacunaInfoDisplay(vac).icon || $icons.vaccine"
                  width="18"
                  height="18"
                />
              </div>
              <div class="vac-nombre-wrap">
                <span class="vac-nombre">{{
                  vacunaInfoDisplay(vac).enfermedad || vac.vacuna?.nombre || "—"
                }}</span>
                <span class="vac-desc">{{
                  vacunaInfoDisplay(vac).descripcionCorta ||
                  vac.vacuna?.descripcion ||
                  ""
                }}</span>
              </div>
            </div>
            ...
            <!-- Fechas -->
            <span
              class="vac-col vac-fecha"
              :data-label="t('petDetail.colLastDose')"
              >{{ fmt(vac.fecha_aplicacion) }}</span
            >
            <span
              class="vac-col vac-fecha"
              :class="{ 'vac-fecha--alert': vac.estado === 'retrasada' }"
              :data-label="t('petDetail.colNextDose')"
            >
            ...
            <div class="vac-col vac-col-acciones">
              ...
              >
                <span
                  v-if="marcandoId === vac.id"
                  class="spinner"
                  style="width: 11px; height: 11px; border-width: 1.5px"
                />
              ...
              <div class="vac-micro-btns">
                <button
                  type="button"
                  class="vac-micro-btn vac-micro-btn--edit"
                  :title="t('common.edit')"
                  @click.stop="abrirEditar(vac)"
                >
                  <Icon :icon="$icons.edit" width="11" height="11" />
                </button>
                <button
                  type="button"
                  class="vac-micro-btn vac-micro-btn--del"
                  :title="t('common.delete')"
                  @click.stop="pedirEliminar(vac)"
                >
            ...
            <div class="vac-col vac-col-bell" style="position: relative">
              <button
                ...
                :title="
                  getDeVacuna(vac.id)
                    ? t('petDetail.reminderActive')
                    : t('petDetail.addReminder')
                "
                :ref="
                  (el) => {
                    if (el) reminderBtnRefs[vac.id] = el;
                  }
                "
      ...
      <Transition name="modal-fade">
        <div
          v-if="confirmEliminar"
          class="vac-confirm-overlay"
          @click.self="confirmEliminar = null"
        >
          <div class="vac-confirm-card card">
            <div class="vac-confirm-icon">
              <Icon :icon="$icons.delete" width="22" height="22" />
            </div>
            <h3 class="vac-confirm-title">
              {{ t("petDetail.deleteVaccineTitle") }}
            </h3>
            <p class="vac-confirm-desc">
              <strong>{{ confirmEliminar.vacuna?.nombre }}</strong>
              {{ t("petDetail.willBeDeleted") }}
            ...
            <div class="vac-confirm-btns">
              <button
                ...
                class="btn btn-ghost"
                :disabled="eliminando"
                @click="confirmEliminar = null"
              >
                {{ t("common.cancel") }}
              </button>
              <button
                type="button"
                class="btn-eliminar-vac btn"
                :disabled="eliminando"
        ...
        <div v-if="proximaVacuna" class="card md-tip md-tip--coral">
          <div class="card-body md-tip-body">
            ...
            <div>
              ...
              <p class="md-tip-text">
                {{ t("petDetail.nextVaccineOf") }}
                <strong>{{ mascota.nombre }}</strong> {{ t("petDetail.is") }}
                <strong>{{ proximaVacuna.vacuna?.nombre }}</strong
                >. {{ t("petDetail.expectedDate") }}:
                {{ fmt(proximaVacuna.proxima_aplicacion) }}.
              </p>
            </div>
          </div>
        </div>
...
.vac-row--retrasada {
  background: rgba(253, 234, 234, 0.4);
}
...
.vac-col-nombre:hover .vac-nombre {
  color: var(--color-teal-dark);
}
...
.vac-row-icon--pendiente {
  background: #fef9e7;
  color: #9a6a10;
}
.vac-row-icon--retrasada {
  background: var(--color-danger-light);
  color: var(--color-danger);
}
...
@media (max-width: 860px) {
  ...
  .vac-row--pendiente {
    background: rgba(254, 249, 231, 0.6) !important;
  }
  .vac-row--retrasada {
    background: rgba(253, 234, 234, 0.6) !important;
  }
...
.btn-eliminar-vac:hover:not(:disabled) {
  background: #be4b4b;
  transform: translateY(-1px);
}
.btn-eliminar-vac:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
