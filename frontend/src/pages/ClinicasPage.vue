...
import {
  useClinicas,
  SERVICIOS_META,
  SERVICIOS_FILTRO_KEYS,
} from "@/composables/useClinicas.js";
...
function estrellas(val) {
  ...
  return (
    "★".repeat(llenas) + (media ? "½" : "") + "☆".repeat(5 - llenas - media)
  );
}
...
watch(ubicacion, (u) => {
  ...
  markers.value["_usuario"] = L.marker([u.lat, u.lng], {
    icon: icono,
    zIndexOffset: 1000,
  })
    .addTo(leaflet)
  ...
  <div class="cl-page">
    ...
    <div class="cl-topbar">
      ...
      <div class="cl-topbar-row1">
        <h1 class="cl-titulo">
          {{ t("clinics.availableTitle") }}
          <span v-if="!cargando" class="cl-titulo-count"
            >({{
              t("clinics.foundCount", { n: clinicasFiltradas.length })
            }})</span
          >
        </h1>
        ...
        <div class="cl-search-wrap">
          ...
          <div class="cl-search-input-wrap">
            ...
            <button
              ...
              "
            >
              <Icon :icon="$icons.close" width="12" height="12" />
            </button>
          </div>
        </div>
      ...
      <div class="cl-filtros-row">
        <button
          type="button"
          v-for="key in SERVICIOS_FILTRO_KEYS"
          :key="key"
          class="cl-chip"
        ...
        >
          <Icon
            :icon="SERVICIOS_META[key]?.icon || $icons.clinic"
            width="15"
            height="15"
          />
          {{ t(`clinics.services.${key}`) }}
        </button>
        <button
          type="button"
    ...
    <div class="cl-body">
      ...
      <div
        class="cl-mapa-col"
        :class="{ 'cl-mapa-col--hidden': vistaMovil === 'lista' }"
      >
      ...
      <div
        class="cl-lista-col"
        :class="{ 'cl-lista-col--hidden': vistaMovil === 'mapa' }"
      >
        <!-- Loading skeletons -->
        ...
        <div v-else-if="clinicasFiltradas.length === 0" class="cl-empty">
          <div class="cl-empty-icon">
            <Icon :icon="$icons.clinic" width="36" height="36" />
          </div>
          ...
          >
            ...
            <div class="cl-card-body">
              ...
              <div class="cl-card-rating">
                <span class="cl-stars">
                  <span
                    ...
                    class="cl-star"
                    :class="
                      i <= Math.round(c.valoracion || 0) ? 'cl-star--on' : ''
                    "
                    >★</span
                  >
                </span>
                <span class="cl-rating-val">{{
              ...
              <p class="cl-card-dir">
                ...
                <span v-if="fmtDistancia(c.distancia)" class="cl-dir-dist"
                  >• {{ fmtDistancia(c.distancia) }}</span
                >
              </p>
              ...
              <div class="cl-card-footer">
                <div class="cl-card-chips">
                  <span
                    ...
                    class="cl-chip-mini"
                    :style="{
                      background: SERVICIOS_META[s]?.bg || '#F7F2EA',
                      color: SERVICIOS_META[s]?.color || '#9B8A75',
                    }"
    ...
    <div class="cl-toggle-movil">
      <button
        type="button"
        class="cl-vtoggle"
        :class="{ 'cl-vtoggle--on': vistaMovil === 'lista' }"
      ...
      <button
        type="button"
        class="cl-vtoggle"
        :class="{ 'cl-vtoggle--on': vistaMovil === 'mapa' }"
...
.cl-topbar {
  ...
  box-shadow: 0 2px 8px rgba(60, 46, 31, 0.06);
  z-index: 100;
}
...
.cl-search-input:focus {
  border-color: var(--color-teal);
  box-shadow: 0 0 0 3px rgba(124, 203, 194, 0.15);
  background: #fff;
  outline: none;
}
.cl-search-clear {
...
.cl-orden-btn:hover:not(:disabled) {
  border-color: var(--color-teal);
  color: var(--color-teal-dark);
}
.cl-orden-btn--on {
...
.cl-filtros-row::-webkit-scrollbar {
  display: none;
}
...
.cl-chip--on {
  ...
  color: #fff;
  box-shadow: 0 2px 8px rgba(124, 203, 194, 0.35);
}
.cl-chip--reset {
...
.cl-lista-col::-webkit-scrollbar {
  width: 4px;
}
.cl-lista-col::-webkit-scrollbar-thumb {
...
.cl-card--activa {
  border-color: var(--color-primary);
  box-shadow:
    0 0 0 2.5px rgba(240, 130, 99, 0.2),
    var(--shadow-md);
}
...
.cl-badge-24h {
  ...
  padding: 2px 7px;
  border-radius: var(--radius-full);
  box-shadow: 0 1px 4px rgba(240, 130, 99, 0.4);
}
...
.cl-reservar-btn:hover {
  background: var(--color-teal-dark);
  transform: translateY(-1px);
  box-shadow: 0 4px 12px rgba(124, 203, 194, 0.45);
}
...
.cl-vtoggle--on {
  ...
  color: #fff;
  box-shadow: 0 2px 8px rgba(240, 130, 99, 0.3);
}
