...
const info = computed(() =>
  getVacunaInfo(props.vacuna?.vacuna?.nombre, locale.value),
);
const cfg = computed(
  () => estadoConfig[props.vacuna?.estado] || estadoConfig.pendiente,
);
...
const diasInfo = computed(() => {
  ...
  const diff = Math.ceil(
    (new Date(props.vacuna.proxima_aplicacion) - Date.now()) /
      (1000 * 60 * 60 * 24),
  );
  if (diff > 0)
    return { texto: t("petDetail.daysLeft", { n: diff }), positivo: true };
  if (diff === 0) return { texto: t("petDetail.today"), positivo: true };
  return {
    texto: t("petDetail.overdueDays", { n: Math.abs(diff) }),
    positivo: false,
  };
});
      ...
      <Transition name="modal-slide">
        <div v-if="visible" class="mdv-modal card">
          ...
          <div class="mdv-body">
            ...
            <div class="mdv-fechas">
              <div class="mdv-fecha-item">
                <span class="mdv-fecha-label">{{
                  t("petDetail.colLastDose")
                }}</span>
                <span class="mdv-fecha-val">{{
                  fmt(vacuna?.fecha_aplicacion)
                }}</span>
              </div>
              <div class="mdv-fecha-sep" />
              <div class="mdv-fecha-item">
                <span class="mdv-fecha-label">{{
                  t("petDetail.colNextDose")
                }}</span>
                <span
                  class="mdv-fecha-val"
                  :style="{
                    color:
                      diasInfo?.positivo === false ? 'var(--color-danger)' : '',
                  }"
                >
                ...
                <span
                  v-if="diasInfo"
                  class="mdv-dias"
                  :class="{ 'mdv-dias--alert': !diasInfo.positivo }"
                >
                  {{ diasInfo.texto }}
                </span>
              </div>
            ...
            <template v-if="info">
              <div class="mdv-section">
                <p class="mdv-section-label">
                  {{ t("petDetail.whatDisease") }}
                </p>
              ...
              <div
                v-if="locale === 'es' && vacuna?.vacuna?.descripcion"
                class="mdv-section"
              >
                <p class="mdv-section-label">
                  {{ t("petDetail.fullDescription") }}
                </p>
              ...
              <div class="mdv-row-2">
                <div class="mdv-section mdv-section--card">
                  <p class="mdv-section-label">
                    {{ t("petDetail.frequency") }}
                  </p>
                  <p class="mdv-section-text">{{ info.frecuencia }}</p>
                </div>
                <div class="mdv-section mdv-section--card">
                  <p class="mdv-section-label">
                    {{ t("petDetail.riskLevel") }}
                  </p>
                  <span
                    class="mdv-riesgo"
          ...
          <div class="mdv-footer">
            <button type="button" class="btn btn-teal" @click="$emit('close')">
              {{ t("common.close") }}
            </button>
          </div>
        </div>
