...
const razasFiltradas = computed(() => {
  ...
  return props.razas.filter((r) =>
    `${r.nombre || ""} ${nombreRaza(r)}`.toLowerCase().includes(q),
  );
});
...
<template>
  <div class="rs-wrap" ref="wrapRef" :class="{ 'rs-wrap--disabled': disabled }">
    ...
    >
      <span v-if="seleccionada" class="rs-selected">{{
        nombreRaza(seleccionada)
      }}</span>
        ...
        <div v-if="razasFiltradas.length > 0" class="rs-footer">
          ...
          {{
            razasFiltradas.length === 1
              ? t("pets.breed").toLowerCase()
              : t("pets.breeds").toLowerCase()
          }}
        </div>
      </div>
    </Transition>
...
.rs-list::-webkit-scrollbar {
  width: 4px;
}
.rs-list::-webkit-scrollbar-track {
  background: transparent;
}
