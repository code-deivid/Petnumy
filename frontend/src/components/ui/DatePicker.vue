...
const localeKey = computed(() =>
  locale.value?.startsWith("en")
    ? "en"
    : locale.value?.startsWith("va")
      ? "va"
      : "es",
);
const MESES = computed(() =>
  normalizarArrayI18n("datePicker.months", FALLBACK_MONTHS[localeKey.value]),
);
const DIAS = computed(() =>
  normalizarArrayI18n("datePicker.days", FALLBACK_DAYS[localeKey.value]),
);
...
const etiqueta = computed(() => {
  ...
  return seleccionada.value.toLocaleDateString(
...
function esSeleccionado(dia) {
  ...
  return (
    dia === s.getDate() &&
    vistaM.value === s.getMonth() &&
    vistaAño.value === s.getFullYear()
  );
}
function esHoy(dia) {
  if (!dia) return false;
  return (
    dia === hoy.getDate() &&
    vistaM.value === hoy.getMonth() &&
    vistaAño.value === hoy.getFullYear()
  );
}
...
function onScrollResize() {
  if (abierto.value) recalcularPos();
}
...
<template>
  <div class="dp-wrap" ref="wrapRef">
    ...
    <div
      ...
      class="dp-trigger"
      :class="{
        'dp-trigger--open': abierto,
        'dp-trigger--filled': !!modelValue,
      }"
        ...
        >
          ...
          <div class="dp-header">
            <button
              ...
              class="dp-nav-btn"
              @click="
                modoPicker === 'dias' ? irMesAnterior() : (vistaAño2 -= 16)
              "
            >
            ...
            <div class="dp-header-labels">
              ...
              <button
                ...
                class="dp-header-btn"
                @click="
                  modoPicker = modoPicker === 'años' ? 'dias' : 'años';
                  vistaAño2 = vistaAño;
                "
              >
                {{ vistaAño }}
            ...
            <button
              ...
              class="dp-nav-btn"
              @click="
                modoPicker === 'dias' ? irMesSiguiente() : (vistaAño2 += 16)
              "
            >
