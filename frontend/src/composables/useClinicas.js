// src/composables/useClinicas.js
// ============================================================
...
const GEOCODIGOS = {
  ...
  '46004': { lat: 39.4728, lng: -0.3810, label: 'Extramurs' },
  '46005': { lat: 39.4672, lng: -0.3820, label: 'Eixample Norte' },
  ...
  '46023': { lat: 39.4600, lng: -0.3351, label: 'El Cabanyal' },
  ...
  'extramurs':    { lat: 39.4728, lng: -0.3810, label: 'Extramurs' },
...
function geocodificarZona(query) {
  ...
  // Búsqueda parcial
  const key = Object.keys(GEOCODIGOS).find(k => k.includes(q) || q.includes(k))
...
export function useClinicas() {
  ...
  const clinicasConDistancia = computed(() =>
    clinicas.value.map(c => ({
      ...c,
      distancia: (ubicacion.value && c.latitud && c.longitud)
        ? haversine(ubicacion.value.lat, ubicacion.value.lng, c.latitud, c.longitud)
        : null
    }))
  )
  ...
  async function cargar() {
    ...
    if (!ok) { error.value = data.message || 'Error cargando clínicas'; return }
  ...
  async function usarUbicacionReal() {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ubicacion.value         = { lat: pos.coords.latitude, lng: pos.coords.longitude, label: 'Mi ubicación' }
        filtros.value.orden     = 'distancia'
        buscandoUbicacion.value = false
      },
      err => {
        errorUbicacion.value    = 'No se pudo obtener tu ubicación. Usa la búsqueda manual.'
        buscandoUbicacion.value = false
      },
  ...
  async function buscarPorZona(query) {
    errorUbicacion.value = null
    if (!query?.trim()) { ubicacion.value = null; return }

    const resultado = geocodificarZona(query.trim())
    if (resultado) {
      ubicacion.value         = { lat: resultado.lat, lng: resultado.lng, label: resultado.label }
      filtros.value.orden     = 'distancia'
      return true
    }
    ...
    try {
      ...
      if (data.length > 0) {
        ubicacion.value     = { lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon), label: data[0].display_name.split(',')[0] }
        filtros.value.orden = 'distancia'
        buscandoUbicacion.value = false
        return true
      }
  ...
  function resetFiltros() {
    filtros.value = { servicios: [], abierto24h: false, orden: 'relevancia', busqueda: '' }
  }
